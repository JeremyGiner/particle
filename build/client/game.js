...
particle_Main.main = function() {
	var _oModel = new particle_model_Model();
	var _oView = new particle_view_View(_oModel);
	new particle_controller_Controller(_oModel,_oView);
};
...
particle_controller_Controller.prototype = $extend(particle_controller_AController.prototype,{
	...
	,processGameStep: function() {
		...
		if(this._oModel.getSpeed() > 0) {
			this._oTimer = haxe_Timer.delay($bind(this,this.processGameStep),Math.ceil(400 / this._oModel.getSpeed()));
		}
		this._bProcessing = false;
	}
});
var particle_controller_DragDrop = function(oModel,oView,oMove) {
	...
	var oCanvas = this._oView.getCanvas();
	oCanvas.addEventListener("click",function(oEvent) {
		if(oEvent.button == 2) {
			_gthis._oModel.setSelection(null);
			return;
		}
		var oPosition = _gthis._oView.toGridPosition(oEvent.pageX,oEvent.pageY);
		var oTargetParticle = _gthis._oModel.getParticleByPosition(oPosition);
		if(_gthis._oModel.getSelection() != null && oTargetParticle == null) {
			_gthis._oModel.setParticlePosition(_gthis._oModel.getSelection(),oPosition);
			return;
		}
		_gthis._oModel.setSelection(oTargetParticle);
	});
	oCanvas.addEventListener("mousemove",function(oEvent1) {
		var oPosition1 = _gthis._oView.toGridPosition(oEvent1.clientX,oEvent1.clientY);
		_gthis._oView.getMenu().setPosition(oPosition1);
		_gthis._oView.getMenu().update();
	});
	this._oModel.onSelectionChange.add(function(oParticle1) {
		_gthis._oView.updateSelection();
	});
};
	...
	process: function() {
		...
		while(oParticle.hasNext()) {
			...
			while(_g < _g1.length) {
				...
				var oTarget = this._oModel.getParticleByPosition(oSpawnPosition);
				if(oTarget != null) {
...
var particle_model_Model = function() {
	...
	this.onSelectionChange = new trigger_EventListener();
...
particle_model_Model.prototype = {
	...
	,setSelection: function(oParticle) {
		this._oSelected = oParticle;
		this.onSelectionChange.trigger(this._oSelected);
	}
	,setSpeed: function(i) {
		this._iSpeed = i;
	...
	,removeParticle: function(oParticle) {
		if(oParticle.getId() == null) {
			console.log("src/particle/model/Model.hx:168:","Warning: trying to remove particle with no identity");
			return;
		}
		console.log("src/particle/model/Model.hx:176:","removing #" + oParticle.getId());
		console.log("src/particle/model/Model.hx:177:",haxe_CallStack.callStack());
...
var particle_view_Menu = function(oModel,oContainer) {
	...
	this._oPosition = new space_Vector2i();
};
...
particle_view_Menu.prototype = {
	...
	,getData: function() {
		return { speed : this._oModel.getSpeed(), selected : this._oModel.getSelection() == null ? null : this._oModel.getSelection().getId(), position : this._oPosition};
	}
	,update: function() {
...
var particle_view_View = function(oModel) {
	this._oDragged = null;
	var _gthis = this;
	var oCanvas = window.document.getElementById("canva-game");
	this._oPixi = new PIXI.Application({ width : oCanvas.clientWidth, height : oCanvas.clientHeight, view : oCanvas});
...
particle_view_View.prototype = {
	...
	,toGridPosition: function(x,y) {
		var rx = window.innerWidth / this._oPixi.screen.width;
		var ry = window.innerHeight / this._oPixi.screen.height;
		return new space_Vector2i(Math.floor((rx * x - this._oStage.x) / this._oStage.scale.x),Math.floor((ry * y - this._oStage.y) / this._oStage.scale.y));
	}
	,getCanvas: function() {
		return this._oPixi.view;
	}
	,getMenu: function() {
		return this._oMenu;
	}
